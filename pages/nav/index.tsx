...
export default class Nav extends React.Component<any, any> {
      ...
      dropitem: [
        {
          name: 'viser', // item名
          image:
            'https://gw.alipayobjects.com/os/s/prod/antv/assets/image/icon/g2-c94ef.svg', // 图片
        ...
        },
        {
          name: 'viser-graph',
          image:
            'https://gw.alipayobjects.com/os/s/prod/antv/assets/image/icon/g6-b4554.svg',
          completed: true,
          keys: 'viserGraph',
        },
        {
          name: 'viser-cell',
          image:
            'https://gw.alipayobjects.com/os/s/prod/antv/assets/image/icon/f2-d360c.svg',
          completed: false,
          keys: null,
        },
      ],
  ...
  render() {
    return (
      <div className={`viser-dropdown`}>
        <a className="visble-tit" onClick={this.handleVisible}>
          {this.state.selected}
        </a>
        <div
          className="viser-dropmenu"
        ...
        >
          {this.state.dropitem.map((item: any, index: number) => (
            <a
              ...
              key={index}
              onClick={e =>
                this.handleClick(e, item.name, item.completed, item.keys)
              }
            >
              {item.image && (
                <i
              ...
              {!item.completed && (
                <span>
                  {this.props.pageLan === 'cn' ? '建设中' : 'constructing'}
                </span>
              )}
            </a>
          ))}
