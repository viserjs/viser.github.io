...
export default class App extends React.Component {
    ...
    render() {
        ...
        return <div>
            <div id="mountNode">
                <h4 style={{ textAlign: 'center', marginBottom: 5 }}>北京市 2010-2015 年 AQI 指数</h4>
                {data.length !== 0 && (
                    <Chart
                        forceFit={true}
                        height={400}
                    ...
                    >
                        <Tooltip />
                        <Axis />
                        <Line position="date*aqi" opacity={.75} />
                        {ticks.map((tick, i) => {
                            const nextTick = ticks[i + 1];
                            if (nextTick) {
                                return <Guide type="region" start={['min', tick]} end={['max', nextTick]} style={{ fill: colors[i], fillOpacity: 0.4 }} />
                            }
                        })}
                    </Chart>
                )}
