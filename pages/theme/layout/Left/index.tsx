        ...
        return <div className="theme-left theme-pannel">
            <TabSld title={getTransText('function', pageLan)} visible={true} >
                <div className="btn-group">
                    ...
                    <Button type="default" icon="import" className="no-right-radius no-right-border">
                        {getTransText('function/import', pageLan)}
                    </Button>
                    <Button type="default" icon="export" className="no-left-radius" onClick={this.handleDownload}>
                        {getTransText('function/export', pageLan)}
                    </Button>
                </div>
                <div className="btn-group">
                    <Button type="default" icon="refresh" className="no-right-radius no-right-border" onClick={setData}>
                ...
                <div>
                    <Input type="text" label={getTransText('function/themname', pageLan)} />
                    <Input type="number" label={getTransText('function/seriesNum', pageLan)} />
                </div>
                <hr />
                <h5>默认方案</h5>
                <div className="clearfix">
                    {colors && colors.length !== 0 && colors.map((color: any, key: number) => (
                        <Col span={color.colors.length > 8 ? 24 : 12} key={key}>
                            <ColorBar colors={color.colors} bgColor={color.background} blockWidth={color.colors.length > 16 ? 15 : 20} onClick={this.handleColorClick} />
                        </Col>
                    ))}
                </div>
            </TabSld>
...
const Left = connect(mapState, mapDispatch)(App);
export default Left;